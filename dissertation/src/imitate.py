...
class ImitateHeadPose:
    ...
    def __init__(self):
        ...
        self.pose_lock_threshold = 0.3  # Looser threshold now (0.3 radians)
        ...
        while not rospy.is_shutdown():
            ...
            while not mimic_success and not rospy.is_shutdown():
                ...
                if self.new_frame:
                    ...
                    if yaw_diff < 0.3 and pitch_diff < 0.3:
                        mimic_time += self.TICK
                        if mimic_time >= 3.0:  # User is in position for 3 seconds
                            mimic_success = True
                            rospy.loginfo("Mimic success!")
                    else:
                        mimic_time = 0.0  # Reset mimic time if out of range
